...
function save() {
    ...
    var url,alert_text;
    if (save_method==='add') {
        url = base_url+"menu/ajax_add";
        alert_text = "Data Berhasil Ditambahkan";
    } else {
        url = base_url+"menu/ajax_update";
...
function hapus(id) {
    bootbox.confirm("Are you sure?", function(result) {
        if(result) {
            $.ajax({
                url : base_url+"menu/ajax_delete/"+id,
                type: "POST",
                data: $('#form').serialize(),
                dataType: "JSON",
                success: function(data)
...
function update(id)
{
    ...
    $.ajax({
        url : base_url+"menu/ajax_edit/"+id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
            $('[name="id_pk"]').val(data.menus_id);
            $('[name="name"]').val(data.name);
            $('[name="url"]').val(data.url);
            $('[name="title"]').val(data.title);
            $('[name="parent_id"]').val(data.parent_id);
            $('#enable'+data.enable).attr("checked",'checked');
