...
function save() {
    ...
    var url,alert_text;
    if (save_method==='add') {
        url = base_url+"akun/ajax_add";
        alert_text = "Data Berhasil Ditambahkan";
    } else {
        url = base_url+"akun/ajax_update";
...
function hapus(id) {
    bootbox.confirm("Are you sure?", function(result) {
        if(result) {
            $.ajax({
                url : base_url+"akun/ajax_delete/"+id,
                type: "POST",
                data: $('#form').serialize(),
                dataType: "JSON",
                success: function(data)
...
function update(id)
{
    ...
    $.ajax({
        url : base_url+"akun/ajax_edit/"+id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
            $('[name="id_pk"]').val(data.USER_ID);
            $('[name="username"]').val(data.USERNAME);
            $('[name="nama_lengkap"]').val(data.NAMA_LENGKAP);
            $('[name="alamat"]').val(data.ALAMAT);
            $('[name="password"]').val('noset');
            $('#blokir'+data.BLOKIR).attr("checked",'checked');
...
function bulk_delete() {
    ...
    if (list_id.length > 0) {
        ...
        if(result) {
            $.ajax({
                url : base_url+"akun/ajax_bulk_delete/",
                type: "POST",
                data: {id:list_id},
                dataType: "JSON",
                success: function(data)
