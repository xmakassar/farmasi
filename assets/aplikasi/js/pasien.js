$(document).ready(function(){
   //datatables
    ...
    table2 = $("#table_download").DataTable({

    });

    table3 = $("#table_total").DataTable({
...
function save() {
    ...
    var url,alert_text;
    if (save_method==='add') {
        url = base_url+"pasien/ajax_add";
        alert_text = "Data Berhasil Ditambahkan";
    } else {
        url = base_url+"pasien/ajax_update";
...
function hapus(id) {
    bootbox.confirm("Yakin Akan Menghapus Data?", function(result) {
        if(result) {
            $.ajax({
                url : base_url+"pasien/ajax_delete/"+id,
                type: "POST",
                data: $('#form').serialize(),
                dataType: "JSON",
                success: function(data)
                {
                    ...
                    if(data.status) //if success close modal and reload ajax table
                    {
                        ...
                        bootbox.alert("Hanya admin yang bisa menghapus data");
                    }
                },
                error: function (e)
                {
...
function update(id)
{
    ...
    $.ajax({
        url : base_url+"pasien/ajax_edit/"+id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
            $('[name="id_pk"]').val(data.id);
            $('[name="no_antrian"]').val(data.nomor);
            $('[name="type_pasien"]').val(data.type_pasien);
...
function bulk_delete() {
    ...
    if (list_id.length > 0) {
        ...
        if(result) {
            $.ajax({
                url : base_url+"pasien/ajax_bulk_delete/",
                type: "POST",
                data: {id:list_id},
                dataType: "JSON",
                success: function(data)
...
function print_table() {
    var url = site_url+"pasien/ajax_show";
    var name = "_blank";
    window.open(url,name);
}
function download_table() {
    validate_jam();
    var url = site_url+"pasien/download_txt";
    //var name = "_blank";
    window.open(url);
}

function validate_jam() {
   var d = new Date();
   if (d.getHours()>12) {
     return false;
   } 
